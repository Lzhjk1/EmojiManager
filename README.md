...
- **图片备注**: 为表情设置备注并按备注搜索，每文件夹独立存储跟随移动
...
| **F12** | 显示/隐藏表情选择器（可自定义） |
| **点击表情** | 复制表情并自动粘贴到QQ |
| **右键表情** | 打开文件位置、删除文件或设置/清除备注 |
| **钉住窗口** | 点击主界面右上角的钉住窗口可以让窗体保持前置 |
| **拖拽图片** | 从QQ或文件夹拖拽图片到主窗口添加表情（需要钉住窗口） |
| **托盘图标** | 左键显示/隐藏，右键退出程序 |
...
- **剪贴板兼容**: 启用怀旧版QQ兼容模式（默认关闭）
- **格式修正**: 一键修复文件扩展名问题
- **重置缩放**: 一键重置所有分组的缩放倍率
- **备注维护**: 一键清理对应图片已不存在的失效备注
...
### 文件名搜索功能
- **默认状态**: 文件名搜索功能默认关闭，只能按文件夹名搜索
- **启用条件**: 适用于有意义文件名的表情包（如"陪笑.jpg"、"喜欢.gif"）
- **使用场景**: 当您的表情文件有描述性名称时可以开启此功能
- **注意事项**: 如果表情文件是QQ默认的32位MD5名称没有整理，建议保持关闭，否则会搜索出很多无关图片
...
**Q: 怀旧版QQ粘贴表情失败？**  
A: 在设置中启用“剪贴板兼容”选项（会同时写入图片数据以兼容怀旧版QQ）。
...
## 📝 版本历史
- **v1.2.5** - 图片备注功能
  - 新增表情包图片备注功能，右键图片即可设置/清除备注
  - 搜索框支持按备注内容匹配，鼠标悬停图片时在状态栏显示备注
  - 备注按文件夹单独存储为 `emoji_remarks.json`，可随表情包文件夹移动/改名而保留
  - 设置中新增"清理失效备注"功能，可一键清理对应图片不存在的孤儿备注

- **v1.2.4** - 页面刷新优化
  - 更新最近表情时只进行局部更新，避免整体页面重载并提升性能

- **v1.2.3** - 细节调整
  - 修复设置页面按钮样式
  - 修正文件扩展名功能添加进度指示
  - 怀旧版QQ兼容功能默认关闭，需要在设置中手动启用（QQNT用户不建议启用）

- **v1.2.2** - 优化运行效率
  - 扫描与刷新移动到后台线程并去抖
  - 窗体位置与尺寸状态仅在失焦或退出时保存
  - 判断图片格式时仅读取文件头部，不再全量读入内存

- **v1.2.1** - 优化内存占用
  - 调整页面的图片加载逻辑，只在视口附近挂载图片，离开后延迟释放
  - 限制同时活跃的图片数量
  - 仅在展开时渲染图片与子目录，折叠时释放资源
